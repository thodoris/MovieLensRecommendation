...
(function (define) {
	...
	define([], function () {
		...
		var pack = function pack(json, options) {
			...
			var ast = function recursiveAstBuilder(item) {
				...
				// Default
				throw new Error('Unexpected argument of type ' + (typeof item === 'undefined' ? 'undefined' : _typeof(item)));
			}(json);
   ...
   */
	...
	if (typeof exports != 'undefined') for (var key in jsonpack) {
		...
		var window = self; //#thodoris : hack to be able to use it in Web Worker
