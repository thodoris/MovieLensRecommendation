...
MOVIEAPP.siteConfig = function () {
        ...
        COMPATIBILITY: {
         ...
         API: {
               // baseurl:'http://62.217.127.19:8080/MovielensDbApi/webresources/',  //base url of the MovieLens API service
                baseurl:'http://localhost:53150/api/',
     ...
     * @returns
     */
    function getApiURL(entitytype,method="",id=""){
 
    switch (entitytype) {
        case 'movie':
        case 'ratings':
            const cMethod =  ( (String.isNullOrEmpty(method)) && (Number.isInteger(id) && id>0  ) )? id : method.toLowerCase()+(MOVIEAPP.helpers.capitalize(entitytype)+"s").replace('ss','s');
